...
function exibaMensagem(titulo, mensagem, limpar, reload, link) {
	...
	bootbox.dialog({
		...
		buttons: {
			success: {
				label: "<i class='far fa-check-circle mr-1'></i>OK",
				className: "btn-primary",
				callback: function() {
					fecharAguarde();
					if (limpar) {
...
function sair() {
	bootbox.confirm({
	    ...
	    buttons: {
	        ...
	        confirm: {
	            label: '<i class="fas fa-power-off mr-1"></i> Sair',
				className: 'btn-danger'
	        }
	    },
	    callback: function (result) {
...
function atualizarBagdeCardPorIdLinha(id) {
	let trId = "#ln"+id;
	let oTable = $(trId).parents("table").DataTable();
	let badge = $(trId).parents(".card").find(".badge");
	if (badge.length > 0) {
		badge.text(oTable.rows().count());
	}
}
